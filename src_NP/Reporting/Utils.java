...
import static HTMLparsePkg.GenerateFailReport.genarateFailureReport;
import HTMLparsePkg.HtmlParse;
import HTMLparsePkg.UpdateTestCaseDesciption;
...
public class Utils {
    ...
    public static void main(String ar[]) throws IOException {
        ...
        int LatestRun = Utils.getLastRun(Reports_path);
     ...
     //   GenerateFailReport.genarateFailureReport(new File(htmlReport), Reports_path + "Results\\Run_" + String.valueOf(LatestRun));
        ...
        if (success == 0) {
            ...
            Utils.getpaths(Reports_path + "\\Results\\Run_" + String.valueOf(LatestRun));
            try {
                Utils.replaceMainTable(false, new File(Reports_path + "index.html"));
                Utils.replaceMainTable(true, new File(Reports_path + "results\\" + "ConsolidatedPage.html"));
                Utils.replaceMainTable(true, new File(Reports_path + "Results\\Run_" + String.valueOf(LatestRun) + "CurrentRun.html"));
                
                fileList.add(new File(Reports_path + "\\Results\\Run_" + String.valueOf(LatestRun) + "CurrentRun.html"));
                for (File f : fileList) {
                    Utils.replaceMainTable(true, f);
                }
     ...
     //   HtmlParse.replaceMainTable(false, new File(Last_run_path + "/CurrentRun.html"));
            ...
            if (temp_eml.length() > 1) {
                ...
                //    SendingEmail.sendmail(to_list, JenkinsURL, Reports_path + "ISTF_Reports.zip", Last_run_path + "screenshot.png");
